...
def PageRank(A, d=0.85, eps=0.0005, maxIterations=1000, verbose=False):
    ...
    while np.linalg.norm(vOld - vNew) >= eps:
        ...
        if i == maxIterations:
            print(
                "The PageRank algorithm ran for",
                maxIterations,
                "with error",
                np.round(np.linalg.norm(vOld - vNew), 3),
            )
...
rankedPages = pd.DataFrame(columns=["Type", "Source", "Destination"])
# add the top 10-ranked webpages
for i in range(10):
    row = data.loc[(data["Type"] == "n") & (data["Source"] == ranks[i])]
