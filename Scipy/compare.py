...
results = pickle.load(open("compare_optimizers_py%s.pkl" % sys.version_info[0], "rb"))
...
for n_dim_index, ((n_dim, n_dim_bench), color) in enumerate(zip(sorted(results.items()), colors)):
...
plt.legend(
    loc=(0.47, 0.07), numpoints=1, handletextpad=0, prop=dict(size=12), frameon=False, ncol=2
)
plt.xlim(-0.2, n_methods - 0.5)
