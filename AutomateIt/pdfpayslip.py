...
class PaySlip(FPDF, HTMLMixin):
    def footer(self):
        ...
        self.cell(0, 10, 'Page %s' % self.page_no(), 0,new_x = XPos.RIGHT,new_y = YPos.TOP, align='C')
...
def generate_payslip(data):
    ...
    pdf.cell(200, 10, txt="Pay Slip for %s, %s" %(month, year), align=Align.C)
    pdf.cell(50)
    pdf.cell(100, 10, txt="Employeed Id: %s" % data['id'],new_x = XPos.LMARGIN,new_y = YPos.NEXT, align='L')
    pdf.cell(50)
    pdf.cell(100, 10, txt="Employeed Name: %s" %data['name'],  align='L')
    html = """
